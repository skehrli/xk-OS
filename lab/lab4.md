# Lab 4: Filesystem
## Design Doc Due: 11/27/23 at 11:59pm. No late days.
## Lab Due: 12/8/23 at 11:59pm.
...
#### Exercise
Modify `writei` and `iopen` to support file writes and file creations. 
...
When you are done, you should be able to pass all tests from `lab4test_a`
...
```
#### Question #1
Explain how `dirlookup` works in details, including its interaction with deleted files.
...
#### Exercise
Build a crash-safe file system. Run `python3 crash_safety_test.py`. It should
print out `file system is crash-safe`. You can find the crash safe test output in `output.txt`
